...
| 22 | → desktop API | `apps/server/src/routes/auth.ts` (Stage 2), `apps/server/src/routes/desktop.ts` (Stage 1-FIX + Stage 2 split) | server | HORIZON_BOOTSTRAP_TOKEN env + operator session bearer | Stage 2 unit + end-to-end integration tests | **integration_verified** (auth surface + bootstrap-safe surface) / **absent** (per-screen data endpoints — Stage 3) | design and implement authenticated read APIs for the 15 remaining screens |
