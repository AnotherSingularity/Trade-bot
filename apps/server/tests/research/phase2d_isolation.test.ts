...
describe('Phase 2D §N — microstructure isolation', () => {
  it('no champion source file imports from src/research/microstructure/*', () => {
    const offenders: string[] = [];
    for (const root of CHAMPION_ROOTS) {
      const dir = join(SERVER_SRC, root);
      for (const file of walk(dir)) {
        const src = readFileSync(file, 'utf8');
        if (/from\s+['"][^'"]*\/research\/microstructure\/[^'"]*['"]/.test(src)) {
  ...
  it('microstructure observer files do not import champion strategy behavior', () => {
    const forbidden = [
    ...
    const offenders: string[] = [];
    for (const file of walk(join(SERVER_SRC, 'research', 'microstructure'))) {
      const src = readFileSync(file, 'utf8');
  ...
  it('microstructure observer cannot write any champion economic table', () => {
  ...
  it('microstructure observer never sets a multiplier above 1', () => {
    const offenders: string[] = [];
    for (const file of walk(join(SERVER_SRC, 'research', 'microstructure'))) {
      const src = readFileSync(file, 'utf8');
      if (/\bsizeMultiplier\s*[:=]\s*(?:[2-9](?:\.\d+)?|1\.[1-9])/.test(src)) {
        offenders.push(relative(SERVER_SRC, file).split(sep).join('/'));
      }
    }
    expect(offenders).toEqual([]);
  ...
  it('microstructure observer contains no createOrder / fetch / order endpoint refs', () => {
    for (const f of walk(join(SERVER_SRC, 'research', 'microstructure'))) {
